// frontend/src/components/TenantBrandingLoader.tsx
// Carrega o branding do servidor toda vez que o app inicia.
// É a fonte de verdade — sobrescreve qualquer cache local desatualizado.
"use client";

import { useEffect } from "react";
import { useTenantStore } from "@/lib/stores/tenantStore";
import Cookies from "js-cookie";

export function TenantBrandingLoader() {
  const { setTenant } = useTenantStore();

  useEffect(() => {
    const slug = Cookies.get("tenant_slug") || "concurso-demo";

    // Busca branding atualizado do servidor
    fetch(`/api/v1/tenants/by-slug/${slug}`, {
      headers: { "X-Tenant-Slug": slug },
    })
      .then((r) => r.json())
      .then((data) => {
        if (data?.tenant?.id) {
          setTenant(data.tenant);

          // Aplica CSS vars imediatamente
          const branding = data.tenant.branding || {};
          if (branding.primary_color) {
            const hsl = hexToHsl(branding.primary_color);
            document.documentElement.style.setProperty("--primary", hsl);
            document.documentElement.style.setProperty("--ring", hsl);
          }
          if (branding.platform_name) {
            document.title = branding.platform_name;
          }
        }
      })
      .catch(() => {
        // Falhou — ThemeProvider usa o cache do tenantStore
      });
  }, []); // Só executa uma vez na montagem

  return null;
}

function hexToHsl(hex: string): string {
  const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);
  if (!result) return "243 75% 59%";
  let r = parseInt(result[1], 16) / 255;
  let g = parseInt(result[2], 16) / 255;
  let b = parseInt(result[3], 16) / 255;
  const max = Math.max(r, g, b), min = Math.min(r, g, b);
  let h = 0, s = 0;
  const l = (max + min) / 2;
  if (max !== min) {
    const d = max - min;
    s = l > 0.5 ? d / (2 - max - min) : d / (max + min);
    switch (max) {
      case r: h = ((g - b) / d + (g < b ? 6 : 0)) / 6; break;
      case g: h = ((b - r) / d + 2) / 6; break;
      case b: h = ((r - g) / d + 4) / 6; break;
    }
  }
  return `${Math.round(h * 360)} ${Math.round(s * 100)}% ${Math.round(l * 100)}%`;
}
