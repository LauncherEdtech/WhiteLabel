...
export default function SharingSettingsPage() {
    ...
    const [instagram, setInstagram] = useState<string>(b.instagram_handle ?? "");


    useEffect(() => {
        const br = (tenant?.branding ?? {}) as Record<string, any>;
        if (br.capsule_style) setSelected(br.capsule_style);
        if (br.instagram_handle !== undefined) setInstagram(br.instagram_handle ?? "");

    }, [tenant?.id]);

    const save = useMutation({
        mutationFn: () => apiClient.put("/appearance/", { capsule_style: selected, instagram_handle: instagram.replace("@", "").trim() || null, }),
        onSuccess: () => {
            if (tenant) {
                setTenant({
                    ...tenant,
                    branding: { ...tenant.branding, capsule_style: selected, instagram_handle: instagram.replace("@", "").trim() || null } as any,
                });
            }
        ...
        <div className="max-w-3xl space-y-8 pb-10">
            ...
            <div className="space-y-2 pt-4 border-t border-border">
                ...
                <p className="text-sm text-muted-foreground">
                    Aparece no rodapé da cápsula de estudos dos seus alunos.
                </p>
