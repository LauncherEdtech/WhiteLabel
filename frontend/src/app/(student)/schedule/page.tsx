...
//
// OTIMIZAÇÕES (v3):
//   - Waterfall eliminado: schedule-check removido, hasSchedule derivado do
//     cache do React Query sem query separada
//   - courseId derivado inline de coursesData (sem useEffect + setState)
//     → elimina 1 ciclo de render antes de disparar a query do schedule
//   - Sempre busca 42 dias (calendar-safe); list/blocks filtram em memória
//     → troca de view não dispara novo fetch
//   - Todas as mutações invalidam ["schedule", courseId] apenas
//     (["schedule-check"] removido)
...
// Sempre buscamos 42 dias (6 semanas) — suficiente para qualquer mês no
// calendário. list/blocks filtram os primeiros 14 em memória.
const FETCH_DAYS = 42;
const DISPLAY_DAYS_DEFAULT = 14;
...
function ScheduleItemRow({ item, courseId, onCheckin, onUncheckin, loading }: {
  ...
  return (
    <div className={cn(
      ...
      <button
        onClick={() => { if (isDone) onUncheckin(item.id); else if (!isSkipped) onCheckin(item.id, true); }}
              ...
              {items.map((item: any) => {
                ...
                return (
                  <div key={item.id} className={cn(
                    ...
                    {!isDoneOrSkipped && (
                      <button onClick={() => router.push(buildItemUrl(item, courseId))}
                        className="shrink-0 text-primary hover:text-primary/80" title={itemActionLabel(item)}>
...
function ScheduleCalendarView({ days, courseId, onCheckin, onUncheckin, loading }: {
  ...
  const { label: selectedLabel, sub: selectedSub } = selectedDate ? parseDayMeta(selectedDate) : { label: "", sub: "" };
            ...
            return (
              <button key={dateStr} onClick={() => setSelectedDate(dateStr)}
                className={cn(
                  ...
                  isSelected && "bg-primary/5", !isSelected && "hover:bg-muted/40",
                  idx % 7 === 6 && "border-r-0",
                )}
              >
                <span className={cn("h-6 w-6 rounded-full flex items-center justify-center text-xs font-semibold",
                ...
                {typeDots.length > 0 && (
                  <div className="flex items-center gap-0.5 flex-wrap">
                    {typeDots.map(type => <div key={type} className={cn("h-1.5 w-1.5 rounded-full", TYPE_CONFIG[type]?.dot || "bg-muted")} />)}
      ...
      {selectedDate && (
        <div className="space-y-3">
          <div className="flex items-center gap-2 pb-1 border-b border-border">
            ...
            {selectedItems.length === 0 && <p className="ml-auto text-xs text-muted-foreground italic">Sem atividades agendadas</p>}
          </div>
...
function ScheduleView({ courseId }: { courseId: string }) {
  ...
  // Query com mesma key usada por SchedulePage → acerto de cache imediato,
  // sem re-fetch. refetchOnWindowFocus aqui para manter dados frescos.
  const { data, isLoading } = useQuery({
    queryKey: ["schedule", courseId],
    queryFn: () => scheduleApi.get(courseId, FETCH_DAYS),
    enabled: !!courseId,
    staleTime: 30_000,
  ...
  const reorganizeMutation = useMutation({
    ...
    onSuccess: () => { invalidateSchedule(); toast.success("Cronograma reorganizado!"); },
  });

  const deleteMutation = useMutation({
    mutationFn: () => scheduleApi.delete(courseId),
    onSuccess: () => { invalidateSchedule(); toast.success("Cronograma removido."); },
  });

  if (isLoading) return <LoadingSkeleton />;

  // list/blocks mostram apenas os primeiros DISPLAY_DAYS_DEFAULT dias;
  // calendar usa todos os dias (já vêm ordenados por data da API).
  const allDays = data?.days || [];
  const days = view === "calendar" ? allDays : allDays.slice(0, DISPLAY_DAYS_DEFAULT);
    ...
    <div data-onboarding="schedule" className="space-y-5 animate-fade-in">
      ...
      {days.length === 0 && (
        <Card><CardContent className="py-12 text-center">
          ...
          <p className="font-semibold text-foreground">Nenhum item nos próximos {DISPLAY_DAYS_DEFAULT} dias</p>
...
function ScheduleWizard({ courseId, onGenerated }: { courseId: string; onGenerated: () => void }) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      // Invalida apenas ["schedule", courseId] — schedule-check não existe mais
      queryClient.invalidateQueries({ queryKey: ["schedule", courseId] });
      ...
      {step === 3 && (
        <Card><CardContent className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{ label: "Manhã", time: "08:00" }, { label: "Tarde", time: "14:00" }, { label: "Noite", time: "19:00" }, { label: "Madrugada", time: "22:00" }].map(({ label, time }) => (
      ...
      {step === 4 && (
        <Card><CardContent className="p-6 space-y-4">
          ...
          <div className="p-3 rounded-lg bg-muted/40 space-y-1">
            ...
            {targetDate && <p className="text-xs text-primary font-medium">• Prova: {new Date(targetDate + "T12:00:00").toLocaleDateString("pt-BR")}</p>}
          </div>
          <div className="flex gap-2">
...
export default function SchedulePage() {
  // selectedCourseId: null = usa primeiro curso disponível
  const [selectedCourseId, setSelectedCourseId] = useState<string | null>(null);
  const queryClient = useQueryClient();

  // 1ª query: lista de cursos
  ...
  // Derivado inline — sem useEffect, sem ciclo de render extra.
  // activeCourseId fica disponível na mesma renderização em que courses chega.
  const activeCourseId = selectedCourseId || courses[0]?.id || null;

  // 2ª query: schedule (única — compartilhada com ScheduleView via cache hit)
  // Busca 42 dias para ser compatível com a view calendário sem re-fetch.
  const { data: scheduleCheckData, isLoading: scheduleLoading } = useQuery({
    queryKey: ["schedule", activeCourseId],
    queryFn: () => scheduleApi.get(activeCourseId!, FETCH_DAYS),
    enabled: !!activeCourseId,
    staleTime: 30_000,
    refetchOnWindowFocus: false, // ScheduleView controla o refetch com windowFocus
  });

  // hasSchedule derivado do cache — sem estado separado
  const hasSchedule = scheduleCheckData !== undefined
    ? !!scheduleCheckData?.schedule
    : null;

  const isLoading = coursesLoading || (!!activeCourseId && scheduleLoading);

  if (isLoading || (activeCourseId && hasSchedule === null)) {
    return <LoadingSkeleton />;
  }

  const handleCourseChange = (newCourseId: string) => {
    setSelectedCourseId(newCourseId);
    // Não precisa invalidar: query key muda e o React Query busca automaticamente
  };

  const handleGenerated = () => {
    queryClient.invalidateQueries({ queryKey: ["schedule", activeCourseId] });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {!hasSchedule && activeCourseId && (
        <ScheduleStartView courseId={activeCourseId} onGenerated={handleGenerated} />
      )}

      {hasSchedule && activeCourseId && (
        <ScheduleView courseId={activeCourseId} />
      )}
    </div>
  );
}