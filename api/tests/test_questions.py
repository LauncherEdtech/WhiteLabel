...
class TestQuestions:
    ...
    def test_answer_question_correct(self, client, student_headers, db):
        ...
        res = client.post(f"/api/v1/questions/{q.id}/answer",
            json={
            ...
            headers=student_headers)
        assert res.status_code == 200
        assert res.json["result"]["is_correct"] is True
    ...
    def test_answer_question_wrong(self, client, student_headers, db):
        ...
        wrong_key = next(
            k for k in ["a","b","c","d"]
            if k != q.correct_alternative_key
        )
        res = client.post(f"/api/v1/questions/{q.id}/answer",
            json={"chosen_alternative_key": wrong_key},
            headers=student_headers)
        assert res.status_code == 200
        assert res.json["result"]["is_correct"] is False

    def test_my_history(self, client, student_headers):
        res = client.get("/api/v1/questions/my-history",
            headers=student_headers)
        assert res.status_code == 200
...
class TestQuestionSecurity:
    ...
    def test_tenant_isolation(self, client, db):
        ...
        # Cria segundo tenant
        t2 = Tenant(name="Tenant 2", slug="tenant-2", plan="basic",
                    is_active=True, features={}, branding={})
        db.session.add(t2)
        db.session.flush()

        u2 = User(tenant_id=t2.id, name="Aluno2", email="aluno2@t2.com",
                  role="student")
        ...
        # Tenta acessar questões do tenant 1
        q = Question.query.first()
        res2 = client.post(f"/api/v1/questions/{q.id}/answer",
