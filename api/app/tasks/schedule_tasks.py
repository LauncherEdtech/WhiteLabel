...
def nightly_schedule_check(self):
    ...
    3. Marca cronogramas completos (todas as aulas assistidas)
    """
    try:
        from app.extensions import db
        from app.models.schedule import StudySchedule, ScheduleItem
        from app.models.course import LessonProgress, Lesson, Module, Subject
        from app.services.schedule_engine import ScheduleEngine
        from datetime import date

        today_str = date.today().isoformat()
        checked = reorganized = 0

        # Todos os cronogramas ativos
        ...
        db.session.commit()
        logger.info(
            f"nightly_schedule_check: {checked} verificados, {reorganized} reorganizados"
        )
        return {"checked": checked, "reorganized": reorganized}
    ...
    try:
        ...
        for schedule in schedules:
            try:
                ...
                logger.warning(f"Erro ao reorganizar schedule {schedule.id}: {e}")
