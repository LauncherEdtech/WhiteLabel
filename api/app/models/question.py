...
class QuestionSourceType(str, enum.Enum):
    BANK = "bank"  # Questão de concurso enviada pelo produtor
    LESSON = "lesson"  # Questão gerada pelo Gemini a partir de uma aula
...
class Question(BaseModel, TenantMixin):
    ...
    # Cascade: se a aula for deletada, as questões dela somem junto
    lesson_id = Column(
        UUID(as_uuid=False),
    ...
    correct_alternative_key = Column(
    ...
    def __repr__(self):
        return f"<Question {self.id[:8]} [{self.source_type}/{self.difficulty}]>"
...
class QuestionAttempt(BaseModel, TenantMixin):
    ...
    # Valores: "practice" | "simulado" | "schedule" | "review" | "lesson"
