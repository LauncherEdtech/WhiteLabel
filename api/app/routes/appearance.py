...
LOGIN_CONTENT_FIELDS = [
    "login_badge", "login_headline", "login_subtext",
    "login_features", "login_form_title", "login_form_subtitle",
]
...
def update_appearance():
    ...
    if "layout_student" in data:
        if data["layout_student"] not in VALID_STUDENT_LAYOUTS:
            return jsonify({"error": "invalid_layout", "valid": list(VALID_STUDENT_LAYOUTS)}), 400
    ...
    if "layout_producer" in data:
        if data["layout_producer"] not in VALID_PRODUCER_LAYOUTS:
            return jsonify({"error": "invalid_layout", "valid": list(VALID_PRODUCER_LAYOUTS)}), 400
        branding["layout_producer"] = data["layout_producer"]
 
    if "login_layout" in data:
        if data["login_layout"] not in VALID_LOGIN_LAYOUTS:
            return jsonify({"error": "invalid_layout", "valid": list(VALID_LOGIN_LAYOUTS)}), 400
    ...
    return jsonify({
        ...
        )["vars"] if branding.get("color_palette") != "custom" else branding.get("custom_vars", {}),
    }), 200
...
def list_palettes():
    return jsonify({
        key: {
            "key": key, "name": p["name"], "description": p["description"],
            "preview": p["preview"], "dark": p["dark"], "vars": p["vars"],
        }
        for key, p in COLOR_PALETTES.items()
    }), 200